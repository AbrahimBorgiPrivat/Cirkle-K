...
if __name__ == "__main__":
    ...
    runtime_namespace = os.getenv("RUNTIME_NAMESPACE", "create_table_and_views")
    ...
    runtime_files = [item.strip() for item in runtime_files_env.split(",") if item.strip()]

    for filename in runtime_files:
        runtime_json_path = runtime_base / filename
        if not runtime_json_path.exists():
            print(f"[main] Warning: {runtime_json_path} not found, skipping.")
            continue
        run_runner(runner_module, runtime_json_path)
