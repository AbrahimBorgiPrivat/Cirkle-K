...
This one-shot ETL service loads the DAR and DAGI JSON files from `resource/json` into PostgreSQL.
...
4. Ensure `resource/json` contains the downloaded DAR and DAGI files.
...
- The service mounts `resource` into the container, so it reads the same `resource/json` folder as the rest of the project.
