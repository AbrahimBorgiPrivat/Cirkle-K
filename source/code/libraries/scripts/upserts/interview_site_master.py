...
def upsert_csv(create_table_if_not_exist: bool = False):
    ...
    data = load_csvs_as_dicts(file_paths=[csv_path], delimiter=";")

    db_name = "circlek"
    schema_name = "interview"
    table_name = "site_master"
    fields_dict = {
        "site_number": {"type": BIGINT(), "primary_key": True, "autoincrement": False},
        "site_name": {"type": TEXT()},
        "site_city": {"type": TEXT()},
        "site_postal_code": {"type": BIGINT()},
        "site_region": {"type": TEXT()},
        "site_brand": {"type": TEXT()},
        "site_format": {"type": TEXT()},
    }
