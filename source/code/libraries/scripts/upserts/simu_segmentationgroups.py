...
from libraries.utils.db_types import BIGINT, TEXT,JSONB, ARRAY, DOUBLE
...
def convert_upsert(create_table_if_not_exist=False):
    file_path = JSON_DIR
    file_name = "SEGMENTATIONSGROUPS.json"
    segmentationsgroups = load_json_file(file_path,file_name)
    db_name='circlek'
    schema_name='public'
    table_name='segmentationsgroups'
    fields_dict = {
        'id': {"type": BIGINT(), "primary_key": True, "autoincrement": False},
        'name': {"type": TEXT()},
        'description': {"type": TEXT()},
        'avg_txn_per_month': {"type": BIGINT()},
        'product_types': {"type": JSONB()},
        'peak_hours': {"type": ARRAY(BIGINT())},
        'hour_weights': {"type": ARRAY(DOUBLE())}, 
        'weekday_weights': {"type": ARRAY(DOUBLE())}, 
    }
