...
from libraries.classes.db_engine import DatabaseEngine

logger = logging.getLogger(__name__)


def _resolve_postgres_defaults(
    username: str | None,
    password: str | None,
    server: str | None,
    port: int | str | None,
) -> tuple[str, str, str, int | str]:
    if not all([username, password, server, port]):
        env.require_postgres_env()
    return (
        username or env.POSTGRES_USERNAME,
        password or env.POSTGRES_PASSWORD,
        server or env.POSTGRES_HOST,
...
def get_data_from_db(db_name: str,
    ...
    username, password, server, port = _resolve_postgres_defaults(
        username=username,
        password=password,
        server=server,
        port=port,
    ...
    while attempt <= retries:
        try:
            ...
            if query_thread.is_alive():
                logger.warning(
                    "Query attempt %s timed out after %s seconds. Retrying...",
                    attempt + 1,
                    timeout,
                )
                attempt += 1
            ...
            if attempt < retries:
                logger.warning(
                    "Database query failed (attempt %s/%s). Retrying in %s seconds...",
                    attempt + 1,
                    retries,
                    delay,
                )
                ...
                logger.error("Query failed after %s retries. Raising error.", retries)
                raise
...
def ensure_table_structure(db_name: str,
    ...
    try:
        ...
        if not create_table_if_not_exist:
            ...
            logger.info(
                "Table '%s.%s' created with fields: %s",
                schema_name,
                table_name,
