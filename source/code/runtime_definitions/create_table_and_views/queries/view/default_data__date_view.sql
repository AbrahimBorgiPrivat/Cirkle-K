...
DROP VIEW IF EXISTS "default_data"."date_view";
CREATE OR REPLACE VIEW "default_data"."date_view" AS
SELECT date_dim_id,
    date_actual,
    day_name,
    day_of_week,
  ...
  WHERE date_part('year'::text, date_actual) >= 2017::double precision AND date_part('year'::text, date_actual) <= (date_part('year'::text, CURRENT_DATE) + 1::double precision);
RESET search_path;
